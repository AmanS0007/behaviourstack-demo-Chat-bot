...
import ChatPanel from '../chat/ChatPanel'; // NEW: Import ChatPanel
...
function PerformanceDebugFlow({ onExit }) {
  // NEW: Get AI functions
  const { updateContext, setIsChatOpen, sendAutoMessage } = useAI();
  ...
  // NEW: Initialize AI context on mount
  useEffect(() => {
    ...
    setIsChatOpen(true);
  }, [updateContext, setIsChatOpen]);

  // NEW: Update AI context when step changes
  useEffect(() => {
    const stepName = 
      step === 'upload' ? 'campaign-upload' :
      step === 'analyzing' ? 'analyzing-performance' :
      step === 'results' ? 'diagnosis-results' :
      step === 'creative' ? 'creative-recovery' : step;

    console.log('🔧 Performance Debug - Step Update:', stepName);
  ...
  }, [step, campaignData, diagnosis, uploadedFile, generatedVariants, updateContext]);

  // NEW: Send auto-message when diagnosis is complete
  useEffect(() => {
    if (step === 'results' && diagnosis && campaignData) {
      setTimeout(() => {
        sendAutoMessage('diagnosis-complete', AutoMessages.diagnosisComplete);
      }, 1500);
    }
  }, [step, diagnosis, campaignData, sendAutoMessage]);

  // NEW: Send auto-message when AI variants are generated
  useEffect(() => {
    if (generatedVariants.length > 0 && !generating) {
      setTimeout(() => {
        sendAutoMessage('recovery-creatives-generated', AutoMessages.creativesGenerated);
      }, 500);
    }
  }, [generatedVariants, generating, sendAutoMessage]);

  // NEW: Send auto-message when creative recovery page loads
  useEffect(() => {
    if (step === 'creative' && uploadedFile && diagnosis) {
      setTimeout(() => {
        sendAutoMessage('creative-recovery-loaded', AutoMessages.creativeRecoveryPageLoaded);
      }, 800);
    }
  }, [step, uploadedFile, diagnosis, sendAutoMessage]);
  ...
  const handleFileUpload = (e) => {
    ...
    // NEW: Send auto-message about file upload
    setTimeout(() => {
      sendAutoMessage('campaign-uploaded', AutoMessages.campaignUploaded);
    }, 300);
