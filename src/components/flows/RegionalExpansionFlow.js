...
function RegionalExpansionFlow({ onExit }) {
  ...
  useEffect(() => {
    console.log('🌎 Regional Expansion Flow - ENTRY');
    updateContext({
      currentFlow: 'regional-expansion',
      currentStep: 'company-input',
      visibleComponents: {},
      campaignData: { company: null },
      availableActions: []
    });
    setIsChatOpen(true);
  ...
  useEffect(() => {
    const stepName = 
      step === 'input' ? 'company-input' :
      step === 'analyzing' ? 'analyzing-markets' :
      'market-recommendations';

    console.log('🌎 Regional Expansion - Step Update:', stepName);
  ...
  }, [step, companyData, markets, preferenceRating, updateContext]);

  // NEW: Send auto-message when markets are recommended
  useEffect(() => {
    if (step === 'results' && markets.length > 0) {
      setTimeout(() => {
        sendAutoMessage('markets-recommended', AutoMessages.marketsRecommended);
      }, 1500);
    }
  }, [step, markets, sendAutoMessage]);
  ...
  const handleAnalyze = () => {
    ...
    // NEW: Send auto-message about company info submission
    setTimeout(() => {
      sendAutoMessage('company-info-submitted', AutoMessages.companyInfoSubmitted);
    }, 300);
